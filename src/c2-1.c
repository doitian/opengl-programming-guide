...
void init(GLFWwindow *window)
{
  ...
  GLuint vertShader = loadShaderBinary("shaders/uniform-block.vert.spv", GL_VERTEX_SHADER);
  GLuint fragShader = loadShaderBinary("shaders/uniform-block.frag.spv", GL_FRAGMENT_SHADER);
  ...
  glNamedBufferStorage(Buffers[UniformBuffer], sizeof(color), color, 0);
  glBindBufferBase(GL_UNIFORM_BUFFER, 0, Buffers[UniformBuffer]);

  GLint size;
  glGetActiveUniformBlockiv(shaderProgram, 0, GL_UNIFORM_BLOCK_DATA_SIZE, &size);
